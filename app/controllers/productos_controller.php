...
class ProductosController extends AppController {
    ...
    function get_ultimos_precios() {
        Configure::write('debug', '0');
        $this->layout = 'ajax';
        //$consulta=$this->Usuario->find('all');

        if (isset($_REQUEST['producto_id']))
            $filtro = " AND d.producto_id=" . $_REQUEST['producto_id'];
        else
            $filtro = "";

        $conquery = "select d.producto_id,c.compra_id,c.compra_fecha, c.compra_cantidad,c.sucursal_id,c.proveedor_id,
                        p.proveedor_razon_social, d.d_compra_cantidad,d.d_compra_precio 
                       from compras c 
                       inner join detallecompras d on d.compra_id=c.compra_id
                       inner join proveedors p on p.proveedor_id=c.proveedor_id
                       where 0=0 $filtro
                       order by c.compra_fecha desc limit 3
                ";
        ...
        $this->set('total', $count);
        $this->set('datos', $cadena);
        $this->render('respuestas/get_ultimos_precios');
    }
                    ...
                    if ($this->Producto->Save($this->data)) {
                        ...
                        for ($i = 1; $i < 4; $i++) {
                            ...
                            if ($this->Kardex->save($this->data)) {
                                ...
                                $d_compra_id = -1;
                                $sql = "SELECT * FROM public.f_movimientos_insert($d_compra_id,$kardex_id,'NOW',$entrada,$salida,'$tipo',$tipo_id,$costo,0,0,false)";
    ...
    function guardar_precio() {
        ...
        if ($datosSesion) {
            if ($this->data) {
                $precio = $this->data['Producto']['producto_precio'];
                if ($this->Producto->Save($this->data)) {
                    $info = array('success' => true, 'msg' => 'Se actualiz&oacute el precio de venta correctamente.');
                    $this->log("Producto precio $precio actualizado en, Productos por->" . $datosSesion['Usuario']['login'], LOG_DEBUG);
                } else {
                    $info = array('success' => false, 'msg' => 'No se pudo actualizar. Intentelo nuevamente');
                    $this->log("Producto precio $precio no se actualizo en, Productos por->" . $datosSesion['Usuario']['login']);
                }
            }
        }else{
            $info = array('success' => false, 'msg' => 'No tiene una sesion activa. Intentelo nuevamente despues de loguearse.');
            $this->log("Producto precio $precio no se actualizo en, Productos por->" . $datosSesion['Usuario']['login']); 
        }
        $this->set('info', $info);
        $this->render('respuestas/guardar_precio');
    }
